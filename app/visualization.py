...
def create_smart_visualization(question: str, subject: str):
    ...
    try:
        ...
        if subject == "Mathematics":
            ...
            if any(k in question_lower for k in [
                ...
                if circ is not None:
                    ...
                    pad = max(0.05 * r, 0.1)  # Minimal padding to keep diagram visible but reduce white space
                    ...
                    if x_all and y_all:
                        ...
                        pad_x = max((x_max - x_min) * 0.02, 0.1)  # Minimal padding to reduce white space
                        pad_y = max((y_max - y_min) * 0.02, 0.1)  # Minimal padding to reduce white space
        ...
        buf = BytesIO()
        plt.savefig(buf, format='png', dpi=180, facecolor='white', bbox_inches='tight', pad_inches=0.01)
        buf.seek(0)
        plt.close(fig)
        return buf
